...
init_sert() {
  ...
  if [[ -e "${DATA_PATH}/live/${DOMAIN}/privkey.pem" ]] && [[ -e "${DATA_PATH}/live/${DOMAIN}/fullchain.pem" ]]; then
...
DOMAIN=$(hostname)
RCA_KEY_SIZE=4096
DATA_PATH="/var/volumes/certbot/conf"

# проверим на наличие обновленных образов
#echo -e "\e[31m-= Обновление docker-образов =-\e[0m"
#docker pull ms4b/1c-hasp:$(uname -r)
#docker pull ms4b/1c-server:$(echo "${APP_SERVER_VER}")
#docker pull ms4b/1c-postgres:$(echo "${POSTGRES_VER}")
#docker pull ms4b/1c-apache:$(echo "${APP_SERVER_VER}")
#docker pull ms4b/nginx:$(echo "${NGINX_VER}")
#docker pull ms4b/zabbix-agent:$(echo "${ZABBIX_AGENT_VER}")
#docker pull dpage/pgadmin4
#docker pull portainer/portainer
#docker pull certbot/certbot

#echo -e "\e[33m...docker-образы обновлены\e[0m"
